...
// Türkçe büyük/küçük harf ve boşluk farklarını yok sayarak karşılaştırma için normalize eder
function normalizeHeading(s) {
  return s.trim().replace(/\s+/g, ' ').toLocaleLowerCase('tr-TR');
}

// "İçindekiler" ile "Sonuç" arasındaki blok = kesin, elle yazılmış içindekiler listesi.
// Her satır bir başlık; başına ">" konursa alt seviye olur (">> " = 3. seviye gibi).
function extractTocBlock(lines) {
  let startIdx = -1, endIdx = -1;
  for (let i = 0; i < lines.length; i++) {
    const n = normalizeHeading(lines[i]);
    if (n === 'i̇çindekiler' || n === 'içindekiler') { startIdx = i; break; }
  }
  if (startIdx === -1) return null;
  for (let i = startIdx + 1; i < lines.length; i++) {
    const n = normalizeHeading(lines[i]);
    if (n === 'sonuç' || n === 'sonuc') { endIdx = i; break; }
  }
  ...
  for (let i = startIdx + 1; i < endIdx; i++) {
    ...
    const level = Math.min(3, (m[2] ? m[2].length : 0) + 1);
    entries.push({ text: m[3].trim(), level });
  }
  return { entries, contentStart: endIdx + 1 };
}

// İçindekiler'deki her satırı, metin gövdesinde birebir aynı yazılmış satırla eşleştirir
function matchTocToBody(entries, lines, searchFrom) {
  let unmatchedCounter = 0;
  return entries.map(e => {
    const target = normalizeHeading(e.text);
    let foundLine = -1;
    for (let i = searchFrom; i < lines.length; i++) {
      if (normalizeHeading(lines[i]) === target) { foundLine = i; break; }
    }
    if (foundLine === -1) { unmatchedCounter++; foundLine = -unmatchedCounter; }
    return { line: foundLine, level: e.level, title: e.text, unmatched: foundLine < 0 };
  });
}
