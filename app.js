...
async function pushIndexToGithub(srcUrl, headings) {
  ...
  const info = derivePathFromRaw(srcUrl);
  if (!info) throw new Error('Bu kitabın kaynağı bir GitHub raw linki değil, otomatik kaydedilemiyor.');
  const path = info.path.replace(/\.txt$/i, '.index.json');
  const apiUrl = `https://api.github.com/repos/${info.owner}/${info.repo}/contents/${path}`;
  let sha = null;
  try {
    const res = await fetch(apiUrl + `?ref=${info.branch}`, { headers: { Authorization: `token ${cfg.token}` } });
  ...
  const body = { message: 'İçindekiler indeksi güncellendi', content, branch: info.branch };
  if (sha) body.sha = sha;
